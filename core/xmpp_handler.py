...
class IncomingMessage(BaseHandler):
    def post(self):
        ...
        try:
            if len(fields) < 2:
                xmpp.send_message(msg.sender, 'Invalid message. Syntax: <dest - user or group>;<talk or mail(ignore for talk)>;<message>')
            else:
    ...
    def parse_message(self, fields, sender):
        if '@' in fields[0]:
            ...
            users = Listener.all().filter("groups", fields[0].upper()).fetch(limit=None)
            for user in users:
                if user.talk != sender:
                    if len(fields) == 3:
                        self.notify(sender, user, fields[1], '[' + fields[0].upper() + '] ' + fields[2])
                    else:
                        self.notify(sender, user, 'talk', '[' + fields[0].upper() + ']' + fields[1])            

    def notify(self, sender,user, tipo, message):
        if tipo == 'talk':
            xmpp.send_message(user.talk, 'From: ' + sender + ' - ' + message)
        elif tipo == 'mail':
            mail.send_mail(user.mail, user.mail, 'GAE Notify', 'From: ' + sender + ' - ' + message)
